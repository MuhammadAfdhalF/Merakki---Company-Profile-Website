...
import WhyChooseModal from "@/components/admin/why-chooses/WhyChooseModal";
...
export default function WhyChoosesAdminPage() {
  ...
  const [editing, setEditing] = useState<WhyChoose | null>(null);
  ...
  async function fetchList() {
    ...
    try {
      const res = await fetch(`${apiBase}/why-chooses`, {
        headers: {
      ...
      const sorted = [...(data as WhyChoose[])].sort(
  ...
  function openEdit(item: WhyChoose) {
    setMode("edit");
    setEditing(item);
    setOpenModal(true);
  }

  async function onDelete(item: WhyChoose) {
    if (!accessToken) return;
    const ok = confirm(`Hapus Why Choose "${item.title}" ?`);
    if (!ok) return;

    try {
      const res = await fetch(`${apiBase}/why-chooses/${item.id}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold">Why Chooses</h1>
          <p className="mt-2 text-white/60 text-sm">
            List why choose items (admin)
          </p>
                  ...
                  {items.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 text-center">
                        <span
                          className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-xs border ${
                            item.is_active
