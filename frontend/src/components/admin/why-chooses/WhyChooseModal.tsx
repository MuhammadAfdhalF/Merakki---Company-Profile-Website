...
export default function WhyChooseModal({
  ...
  async function handleSave() {
    try {
      ...
      if (!description.trim()) throw new Error("Description wajib diisi");
      ...
      const url =
        mode === "create"
          ? `${apiBase}/why-chooses`
          : `${apiBase}/why-chooses/${initialData?.id}`;
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
          <div>
            <h2 className="text-xl font-semibold">
              {mode === "create" ? "Add Why Choose" : "Edit Why Choose"}
            </h2>
            <p className="text-sm text-white/60 mt-1">
              Isi title, description, lalu simpan.
            </p>
          </div>
